...
namespace infini
{
	...
	class TensorObj : public Object //继承自Object类
    {
		friend class GraphObj; //友元类，允许访问私有成员

	protected: //保护成员，可以被派生类、友元类访问
        int dim;

		DataType dtype; //张量数据类型
		vector<WRef<OperatorObj>> targets;//定义一个WRef类型的vector，存储OperatorObj类的弱引用
		WRef<OperatorObj> source; //定义一个WRef类型的source，存储OperatorObj类的弱引用
        Blob data;
        Runtime runtime;

	private://私有成员，只能在类内部和友元类访问
        Shape shape;
